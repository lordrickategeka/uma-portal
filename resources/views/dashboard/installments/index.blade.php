                            ...
                            <div class="installment-plan mb-4 p-3 border rounded">
                                ...
                                <div class="progress mb-3" style="height: 20px;">
                                    <div class="progress-bar" role="progressbar"
                                        style="width: {{ ($plan->paid_installments / $plan->total_installments) * 100 }}%;"
                                        aria-valuenow="{{ ($plan->paid_installments / $plan->total_installments) * 100 }}"
                                ...
                                <table class="table table-sm">
                                    ...
                                    <tbody>
                                        ...
                                        @for($i = $plan->transactions->count() + 1; $i <= $plan->total_installments; $i++)
                                            <tr>
                                                <td>Installment {{ $i }}</td>
                                                <td>Upcoming</td>
                                ...
                                @if($plan->status != 'completed' && $plan->paid_installments < $plan->total_installments)
                                    <div class="text-end">
                                        @php
                                            // Check if there's a pending transaction for this installment plan
                                        ...
                                        @if ($pendingTransaction)
                                            ...
                                            <a href="{{ route('payments.continue', $pendingTransaction->id) }}" class="btn btn-warning">
                                                Continue Payment
                                            </a>
                                            <small class="d-block mt-1">
                                                You have a pending payment. Click to continue.
                                            </small>
                                        @else
