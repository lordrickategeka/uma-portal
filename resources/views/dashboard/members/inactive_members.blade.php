                ...
                <div class="card-body">
                    ...
                    <div class="mb-4">
                        <form action="{{ route('members.inactive') }}" method="GET" class="row g-3">
                            <div class="col-md-6">
                                <input type="text" name="search" class="form-control" placeholder="Search by name, email or UMA number" value="{{ request('search') }}">
                            </div>
                            <div class="col-md-4">
                                <select name="status" class="form-select">
                                    <option value="all" {{ request('status') == 'all' ? 'selected' : '' }}>All Inactive</option>
                                    <option value="no_plan" {{ request('status') == 'no_plan' ? 'selected' : '' }}>No Plan</option>
                                    <option value="expired" {{ request('status') == 'expired' ? 'selected' : '' }}>Expired Plan</option>
                                </select>
                            </div>
                            <div class="col-md-2">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <span class="badge bg-danger">Expired ({{ $user->days_since_expiration }} days ago)</span>
                                        @endif
                                    </td>
                                    <td>
                                        @if($user->userPlan)
                                            ...
                                            <small class="text-muted">
                                                Expired: {{ Carbon\Carbon::parse($user->userPlan->expires_at)->format('M d, Y') }}
                                            </small>
                                        @else
